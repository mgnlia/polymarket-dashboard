...
function computeRewardScore(m: GammaMarket, yesPrice: number): number {
  ...
  const liquidityScore = Math.min(0.25, (m.liquidityNum / 500_000) * 0.25)
  const midnessScore   = 0.20 * (1 - Math.abs(yesPrice - 0.5) * 2)
  const volumeScore    = Math.min(0.15, (m.volume24hr / 5_000_000) * 0.15)
...
export function normaliseMarket(m: GammaMarket): LiveMarket {
  ...
  return {
    ...
    has_rewards:       m.rewardsMinSize > 0,
    rewards_min_size:  m.rewardsMinSize,
    rewards_max_spread: m.rewardsMaxSpread,
...
export async function fetchLiveMarkets(opts: FetchMarketsOptions = {}): Promise<LiveMarket[]> {
  ...
  if (opts.rewardsOnly) params.set('enableOrderBook', 'true')
  ...
  return raw
    .filter(m => !m.archived && !m.restricted || !opts.rewardsOnly)
    .map(normaliseMarket)
}

export async function fetchTopRewardMarkets(limit = 20): Promise<LiveMarket[]> {
  // Fetch more and sort by reward_score
...
export async function fetchTopOpportunities(limit = 10): Promise<LiveMarket[]> {
  ...
  return markets
    .filter(m => m.volume_24h > 10_000 && m.spread > 0.01)
